...
from .hl_tmp_encoder_decoder import HLTmpEncoderDecoder
from copy import deepcopy

@SEGMENTORS.register_module()
class HLCascadeEncoderDecoder(HLTmpEncoderDecoder):
    ...
    def __init__(self,
        ...
        super(HLCascadeEncoderDecoder, self).__init__(
            backbone=backbone,
    ...
    def _init_weight_head(self, decode_head):
        """Initialize ``decode_head``"""
        decode_head[-1].num_classes = 1
        assert isinstance(decode_head, list)
        assert len(decode_head) == self.num_stages
        self.weigh_head = nn.ModuleList()
        for i in range(self.num_stages):
            self.weigh_head.append(builder.build_head(decode_head[i]))
